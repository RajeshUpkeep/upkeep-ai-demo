...
export default function Home() {
  ...
  const [highPriorityWorkOrderIds, setHighPriorityWorkOrderIds] = useState<string[]>([]);
  const [maintenanceDueAssetIds, setMaintenanceDueAssetIds] = useState<string[]>([]);
  ...
  const applyFilters = () => {
    ...
    if (locationFilter) {
      filtered = filtered.filter(wo => 
        wo.location.toLowerCase() === locationFilter.toLowerCase()
      );
    }
    
    setFilteredWorkOrders(filtered);
    setIsFiltered(idFilter !== '' || statusFilter !== '' || locationFilter !== '');
  };
  ...
  const filterHighPriorityWorkOrders = () => {
    clearFilters();
    const filtered = workOrders.filter(wo => wo.priority.toLowerCase() === 'high');
    setFilteredWorkOrders(filtered);
    setIsFiltered(true);
  };
  ...
  const analyzeWorkOrders = async (data: WorkOrder[]) => {
    try {
      ...
      const skillGapWOs = data.filter(
        wo => wo.location === 'San Diego Plant' && wo.status === 'In Progress' && wo.assignedWorkers.length < 2
      );
      ...
      const allInsights: Insight[] = [
        {
          ...
          rootCause: 'It appears to be due to the fact that most work orders have 2 workers assigned to them, but these three at Anaheim Production only have one worker assigned.',
          suggestedAction: 'Would you like me to assign Joe Technician (W010) to these work orders? He has the right skills for these tasks and is currently available at Anaheim Production.',
        ...
        },
        {
          ...
          rootCause: 'These work orders are critical for production equipment at San Diego Plant and Los Angeles Facility. Delays could impact production schedules.',
          suggestedAction: 'Would you like me to notify Maria Rodriguez (W002) and David Johnson (W003) about these high priority tasks? They are the supervisors for these locations.',
        ...
        },
        {
          ...
          rootCause: 'These work orders involve complex mechanical systems that typically require specialized welding skills. Currently, they only have one worker assigned.',
          suggestedAction: 'Would you like me to assign Sarah Williams (W004) to assist with these work orders? She has the necessary welding certification and will be available tomorrow.',
        ...
        {
          id: 4,
          issue: 'Preventive maintenance is due for 3 critical assets at Los Angeles Facility within the next 7 days.',
          rootCause: 'These assets have a maintenance schedule that requires monthly inspection. The last maintenance was performed 23 days ago.',
          suggestedAction: 'Would you like me to schedule preventive maintenance work orders for these assets? I can assign them to David Johnson (W003) who is the supervisor at Los Angeles Facility.',
        ...
        {
          id: 5,
          issue: 'Inventory levels for 2 critical spare parts are below the minimum threshold.',
          rootCause: 'Recent maintenance activities at Anaheim Production have consumed more parts than anticipated. The current stock is insufficient for upcoming scheduled maintenance.',
          suggestedAction: 'Would you like me to generate purchase orders for these parts? Based on historical usage, I recommend ordering 15 units of each part.',
  ...
  const executeHighPriorityAction = async () => {
    try {
      ...
      setTimeout(() => {
        ...
        console.log('Notifications sent to supervisors about high priority work orders');
  ...
  const executeSkillGapAction = async () => {
    try {
      ...
      setTimeout(() => {
        ...
        // Update work orders with new worker
        const updatedWorkOrders = workOrders.map(wo => {
          if (wo.location === 'San Diego Plant' && wo.status === 'In Progress' && wo.assignedWorkers.length < 2) {
            return {
              ...wo,
  ...
  const executePreventiveMaintenanceAction = async () => {
    try {
      ...
      setTimeout(() => {
        ...
        console.log('Preventive maintenance work orders created and assigned to W003');
    ...
    <div className="flex flex-col min-h-screen bg-gray-100">
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-6">
          ...
          <div>
            <div className="mb-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">All Work Orders</h2>
                ...
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  <div className="flex-1 min-w-[200px]">
                    ...
                    >
                      ...
                      <option value="overdue" className="text-gray-800">Overdue</option>
